...
In addition,we also create the following features:
...
n this section we will compare the OSRM *total\_travel\_time* to the taxi *trip\_duration* in our original data. We will specifically focus on understanding those curious cases that suggest significant delays in our data.
...
- Ther percentage of delays during *work* hours is slightly higher but not with practical significance. 
...
The second group of outliers lies at *total\_travel\_time < 10* and *trip\_duration = 600 - 4000*: units in seconds at usual. See the hidden figure or fig 26. Therefore, those are trips over very short distances that should have only taken a few seconds, but in our data they took between 10 minutes and 1.4 hours. In a way, there is a continuum of data points in the lower left to middle part of the plot, but the ones over 10 minutes shows another peak in density:
...
We find:
- The strongest correlations with the *trip\_duration* are seen for the direct *distance* as well tas the *total\_distance* and *total\_travel\_time* derived from the OSRM data. As we have already seen above, the distance are highly correlated with one another and with the *total\_travel\_time*. Also the number of turns, presumably mostly via the *number\_of\_steps*, are having an impact on the *trip\_duration*.
...
# An execution into classification@
...
Recall the distribution of *trip\_durations* from the beginning of this notebook, 
...
We find:
- Our airpot features, *jfk\_trip* and *lg\_trip*, behave as expected and are predominantly slow trips. Also the *blizzard* feature shows a promising increase in slow trips.
...
```{r fig.align="FALSE",warning=FALSE, fig.cap="Fig.33",out.width="100%"}
...
We find:
- As expected, the airports stand out as origins of *slow* rides. Interestingly, a similar observations can be madefor the areas surrounding with airports. 
- There is a number of *fast* trip pickups just across the Hudson river and near Newark airport in Jersey.
...
For a different kind of visualization of your classification problem, I would like to illustrate and *alluvial plot*. Made available through the [*alluvial* package](), those plots are a kind of mix between a flow chart and a bar plot and they show how the target categories relate to various discrete features.
...
In a brief final step, we will feed our exploratory and engineering insights into a simple model to predict the target *trip\_duration* for the *test* data set. However, this is a primary an EDA kernel and not a modelling one. This section leaves plenty of room for improvement for anyone who wants to optimize it. We will be using *XGBoost*, but you can try any other algorithm.
...
pickup_good <- sample_n(pick_good, 5e3)
...
# add dates
combine <- combine %>% 
  mutate(pick_datetime=ymd_hms(pickup_datetime),
         dropoff_datetime=ymd_hms(dropoff_datetime),
         date=date(pickup_datetime))
...
Feature selection is normally an iterative process where we run an initial model with either few or many features and then step-by-step add or remove some features based on the results of the previous run. For the sake of kernel run time here, we only include one model fitting step. You are welcome to use it as a starting point for your analysis and combine it with insgihts from other kernels. We will base our feature selection on the results of the exploratory analysis, in particular the final correlation matrix.

In this code book, the feature selection is structured with a little mode detail than necessary, in order to make it easier to getneralize it to a new problem:
```{r }
...
Finall,y we are performing careful cleaniing step on our *train* sample only. By removing observations from the training data, we always run the risk of overfitting our model to a more idealized version of reality. In particular for the current data set, where lots of values don't make obvious sense, this idelized model might not generalize well to unseen data, which could have similar "quirks". For this reason, we are keeping the validation sample unchanged, so that any noticeable deviations between **train** and **valid** model score can alert us to verfitting. 
