...
```{r echo=FALSE, fig.height=3, fig.width=10, fig.cap="Tuning allows for more flexible patterns to be fit."}
...
p <- ggplot(pdp::boston, aes(crim,cmedv))+
  geom_point(size=1)+
  scale_x_log10("x")+
  scale_y_log10("y")
...
Throughout this guide, we will demonstrate how to tune the different parameters for each model. One way to performing hyperparameter tuning is to fiddle with hyperparameters manually until you find a great combination of hyperparameter values that result in high predictive accuracy. However, this would be very tedious work.

An alternative approach is to peform **grid search**. A __grid search__ is anautomated approach to searching across many combinations of hyperparameter values. Throughout this guide, you will be exposed to different approaches to performing grid searches.

## Cross validation for generalization {#cv}

Our goal is not only find a model that performs well on training data but to find one that performs well on _future unseen data_. So although we can tune our model to reduce some error metric to near zero on our training data. 

Let's go back to this emage
...
The model on the left is considered rigid and consistent. If we provided it to a new training sample, with slightly different values the model would not change that much, if at all. Although it is consistent, the model does not accurately capture the underlying relationship. This is considered a model with high **biase** .
...
To find the model that balances the __biase-variance tradeoff__, we search for a model that minimizes a *k*-fold cross-validation error metric. *k*-fold cross validation is a resampling method that randomly divides the training data into *k* groups (aka folds) of approximately equal size.The model is fit on $k-1$ folds and then the held-out validation fold is used to compute the error. This process is repeated *k* times; each time, a different group of observations is treated as the validation set. This process results in *k* estimates of the test error ($\epsilon_1, \epsilon_2, \dots, \epsilon_k$). Thus, the _k_fold CV estimate is computed by averaging these values, which provides us with an approximation of the error to expect on unseen data.
...
The algorithms  we cover in this guide all have built-in cross validation capabilities. One typically use a 5 or 10 fold CV ($k = 5$ or $k = 10$).  For example, `h2o` implements CV with the `nfolds` argument:
```{r, eval=FALSE}
...
# Regulaization regression {#regularized-regression}


<img src="images/regularization_logo.png"  style="float:right; margin: 0px 0px 0px 0px; width: 50%; height: 50%;" />


Generalized linear models (GLMs) such as ordinary least squares regression and logistic regression are simple and fundamentally approaches for supervised learning. Moreover, when the assumptions required by GMLs are met, the coefficients produced are unbiased and of all unbiased linear techniques, have the lowest variance. 

However, in today's world, data sets beign analyzed typically have a large amount of features. As the number of features grow, our GLM assumptions typically breakdown and our models often overfit (aka have high variance) to the training sample, causing our out of sample error to increase. 

**Regularization** methods provide a means to control our coefficients, which can reduce the variance and decrease out of sample error.

## Prerequisites {#glm-req}
```{block, type="rmdwarning"}
This chapter assumes you are familiar with basic idea behind linear and logstic regression. If not, two tutorials to get you up to speed include [linear regression tutorial](http://uc-r.github.io/linear_regression) and this [logistic regression tutorial](http://uc-r.github.io/logistic_regression).
```
...
library(dplyr) # basic data manipulation procedure
...
However, for many real-life data sets, we have very *wide* data, meaning we have a large number of features (*p*) that we believe are informative in predicting some outcomes. As *p* increases, we can quickly violate some of the OLS assumptions and we require alternative approaches to provide predictive analytic solutions. Specifically, as *p* increases, there are three main issues we most commonly run into:
...
### 3. Interpretability
With a large number of features, we often would like to identify a smaller subset of these features that exhibit the strongest effects. In essence, we sometimes prefere techniques that provide feature selection. 
...
There are three types of penalty parametersx we can implement:
1. Ridge
2. Lasso
...
There are a few packages that impleent variants of regularized regression. You can find a comprehensive list on the [CRAN Machine Learning Task View](https://cran.r-project.org/web/views/MachineLearning.html). However, the most popular implementations which we will cover in this chapter include:
...
A nice vignette is available [here](https://cran.r-project.org/web/packages/glmnet/vignettes/glmnet_beta.pdf). Features include[^ledell]:
...
## Impelementation: regression {#glm-regression}
...
Recall that $\lambda$ is a tuning parameter that helps to control our model from over-fitting to the training data. However, to identify the optimal $\lambda$ value we need  we need to perform [cross-validation](#cv) (CV).  `cv.glmnet` provides a built-in option to perform k-fold CV, and by default, performs 10-fold CV. Here we perform a CV glmnet model for both a ridge and lasso penalty.
...
Now we can iterate over each `alpha` value, apply a CV elastic net, and extract the minimum and one standard error MSE values and respective $\lambda$ values. 
...
Iwe we plot the MSE ± one standard error for the optimal $\lambda$ value for each `alpha` setting, we see that they all fall within the same level of accuracy. Consequently, we could select a full lasso model (`alpha=1`) with $\lambda=0.03521887$, gain the benefits of its feature selection capability and reasonably assume no loss in accuracy.

```{r lmnet-grid-search-results, fig.height=3, fig.width=4, fig.cap="MSE ± one standard error for different alpha penalty parameters."}
...
Regularized regression __assumes a monotonic linear releationship__ between the predictor variables, and the response. The _linear_ relationship part of that statement just means, for a given predictor variable, it assumes for every one unit change in a give predictor variable there is a constant change in the response. This constant change is given by the given coefficient for a predictor. The _monotonic_ relationship means that a given predictor variable will always have a positive or negative relationship. 
...
```{r regress-h2o-init-silenteval, echo=FALSE, cache=FALSE, eval=FALSE}
h2o::h2o.no_progress()
...
```{r regress-h2o-data-prep, eval=FALSE}
...
The following performs as default `h2o.glm` model with `alpha=.5` and it performs a 10 fold cross validation (`nfolds=10`).
...
We can check out more details of the best performing model. Our RMSE (0.12279) is an improvement on our default model (RMSE=0.2015). Also, we can access the optimal model parameters, which show the optimal $\lambda$ value for our model was 0.0279.

```{r regress-h2o-optimal-results, eval=FALSE}
# grab top model id
...
In the __glmnet__ section we discussed how regularized regression assumes a monotonic linear relationship between the predictor variables, and the response. The _linear_ relationship part of that statement just means, for a give predictor variable, it assumes for every one unit change in a given predictor variable there is constant change in the response. This constant change is given by the given coefficient for a predictor. The _monotonic_ relationship means that a given predictor variable will always have a positive or negative relationship.
...
h2o.partialPlot(best_model, data=train_h2o, cols="OVerall-Qual", plot=FALSE) %>% 
  as.data.frame() %>% 
  mutate(
...
```{r}
# conver predicted values to non-transformed
...
```{r glmnet-binary-class-ridge1-results, eval=FALSE}
# lambdas applied to penalty parameter
ridge$lambda %>% head()
...
# x
# <dbl>
# OverTimeYes	1.670349e+00		
# JobInvolvementVery_High	-1.450717e+00		
# JobRoleSales_Representative	1.377262e+00		
# BusinessTravelTravel_Frequently	1.175904e+00		
# JobRoleResearch_Director	-1.038779e+00		
# JobInvolvementHigh	-1.003056e+00		
# EnvironmentSatisfactionVery_High	-9.570745e-01		
# JobSatisfactionVery_High	-9.423902e-01		
# JobRoleLaboratory_Technician	8.838645e-01		
# WorkLifeBalanceBetter	-8.773846e-01
...
# names <chr> x<dbl>
# (Intercept)	-1.647646e+00		
# JobRoleSales_Representative	2.622098e-37		
# OverTimeYes	1.945071e-37		
# JobRoleResearch_Director	-1.545065e-37		
# MaritalStatusSingle	1.391505e-37		
# JobRoleManager	-1.237927e-37		
# BusinessTravelTravel_Frequently	1.156814e-37		
# JobRoleLaboratory_Technician	9.157843e-38		
# JobRoleManufacturing_Director	-8.954434e-38		
# JobInvolvementVery_High	-7.657277e-38	
```
...
elastic1 <- glmnet(train_x, train_y, family="binomial", alpha=.25)
elastic2 <- glmnet(train_x, train_y, family="binomial", alpha=.75)
...
Use caution when including $\alpha=0$ or $\alpha=1$ in the grid search. $\alpha=0$ will produce a dense solution and it can be very slow (or even impossible) to compute in large *N* situations. $\alpha=1$ has no $\ell_2$ penalty, so it is therefore less numerically stable and can be very slow as well as due to slower convergence. If you experience slow computation, we recommend searching across $\alpha$ values of 0.1, 0.25, 0.5, .75, .9.
...
If we plot the deviance of one standard error for the optimal $\lambda$ value for each `alpha` setting. we see that all fall withing the same level of accruacy. Consequently, we could select a full lasso model (`alpha=1`) with $lambda=0.0023$, gain the benefits of its feature selection capability and reasonably assume no loss in accruacy.
...
__Notes__
Ifyou do not include `legacy.axes=TRUE`, in the `plot()` call for the pROC curve, your x-axis will be revealed.
...
```{r h2o-glm-binary-class-int, cache=FALSE}
h2o::h2o.no_progress()
h2o::h2o.init(max_mem_size="5g")
```
...
Next, we do not need to one-hot or standardize our variables as __h2o__ will do this for us. However, we do need to convert our training and test data to __h2o__ objects.

```{r h2o.glm-class-data-prep}
# create training and test ses
library(rsample)
...
# print the MSE and AIC for the validation data 
...
Next, we'll use `h2o.grid` to perform our grid search. The results show that $\alpha=.1$ peformed best: however, the improvmenet is marginal.
...
#### Feature interpreation {#glm-h2o-classification-binary-viz}
##### Feature importance
...
h2o.partialPlot(best_model, data=train_h2o, cols="JobInvolvement", plot=F) %>% 
  as.data.frame() %>% 
...
Similarly, for a continous variable, we can assess the PDP. For example, age is one of the few continuous variables in this data set and we see that our regularized logistic regression model predicts a continuous decreasing probability of attirtion as employees get older. 
...
Since the mnist data contains all numeric predictors (darkness density ranging from 0-255), we do not need to one-hot encode our feature set. Consequently, we only need to convert our features into a matrix and separate the response variable (`V785`). For a multinomial problem our response needs to be either discrete integer values or set as a factor. Since our response values are discrete integer values from 0-9 we can leave them as is.
...
### Basic implemenetation {#classification-multi-glmnet-basic}
...
```{r glmnet-class-multi-ridge-coef, echo=FALSE, out.height="100%", out.width="100%", fig.cap="Coefficients for each multinomial response as $\\lambda$ grows from $0 \\rightarrow\\infity$."}

knitr::include_graphics("images/glmnet-class-multi-ridge-coef.png")
```

Also, the coefficients are stored in a list separated by the response category. We can access these coefficients in a miliar manner as we didthe regression and binary classification coefficients; However, we need to index for the response category of interest. For example, the following gets the coefficicents for response category "g" (note the indexing: <code> coef(ridge)$`9`</code>).
...
Recall that $\lambda$ is a tuning parameter that helps to control our model from over-fitting to the training data. However, to identify the optimal $\lambda$ vakue we need to perform cross-validation with `cv.glmnet` as we did with the regression and binary classification example. However, due to the magnitude of the data slowing __glmnet__ down, rather than perform a full grid search across many alpha settings, we only perform a 5-fold CV glmnet model for three alpha values: 1(ridge), 0.5(elastic net), and 2(lasso).
...
# apply CV ridge regression to mist data
ridge <- cv.glmnet(
  x=train_x,
  y=train_y,
  family="multinomial",
...
Moreover, each response category will use the predictors in different ways. Similar to the binary classification problem, the coefficients represent the change in log-odds probability of the response variable for one unit change in the predictor.  For example, below we see that for the response category "0", a one unit increase in the darkness of the pixel represented by feature V41 causes a 0.0089 log odds increase that the response will be the number "0". A 0.0089 log odds increase translates to $\frac{e^{0.008912499}}{1+e^{0.008912499}} = 0.5022281$ probability increase.
...
Lastly, to assess various performance metrics on our test data we can use `caret::confusionMatrix`, which provides the majority of the performance measures we are typically concerned wit0.9281 and our model does well predicting "0", "1", and "6" but poorly predicting "2", "3", "5","8" and "9".
...
```{r h2o-glm-multinomial-class-int, cache=FALSE}
h2o::h2o.no_progress()
h2o.init(max_mem_size = "5g")
```
...
# set the predictor names
predicotrs <- setdiff(colnames(train),reponse)
```

#### Basic implementation {"h2o-glm-classification"}
...
We can check out more details of the best performiing model. Our AUC (.84) is no better than our default cross-lvaidated model. We can also extract other model parameters, which show the optimal $\lambda$ value for our model was 0.0006.
